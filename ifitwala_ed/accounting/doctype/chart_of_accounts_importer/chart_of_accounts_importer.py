...
def validate_organization(organization):
	parent_organization, allow_account_creation_against_child_organization = frappe.db.get_value('Organization',
		{'name': organization}, ['parent_organization', 'allow_account_creation_against_child_organization'])

	if parent_organization and (not allow_account_creation_against_child_organization):
		msg = _("{} is a child organization.").format(frappe.bold(organization)) + " "
		msg += _("Please import accounts against parent organization or enable {} in organization master.").format(
			frappe.bold('Allow Account Creation Against Child Organization'))
		frappe.throw(msg, title=_('Wrong Organization'))

	if frappe.db.get_all('GL Entry', {"organization": organization}, "name", limit=1):
		return False

@frappe.whitelist()
def import_coa(file_name, organization):
	# delete existing data for accounts
	unset_existing_data(organization)
	...
	# trigger on_update for organization to reset default accounts
	set_default_accounts(organization)
...
def unset_existing_data(organization):
	linked = frappe.db.sql('''select fieldname from tabDocField
		where fieldtype="Link" and options="Account" and parent="Organization"''', as_dict=True)

	# remove accounts data from organization
	update_values = {d.fieldname: '' for d in linked}
	frappe.db.set_value('Organization', organization, update_values, update_values)

	# remove accounts data from various doctypes
	for doctype in ["Account", "Party Account", "Mode of Payment Account", "Tax Withholding Account",
		"Sales Taxes and Charges Template", "Purchase Taxes and Charges Template"]:
		frappe.db.sql('''delete from `tab{0}` where `organization`="%s"''' # nosec
			.format(doctype) % (organization))

def set_default_accounts(organization):
	from ifitwala_ed.setup.doctype.organization.organization import install_country_fixtures
	organization = frappe.get_doc('Organization', organization)
	organization.update({
		"default_receivable_account": frappe.db.get_value("Account",
			{"organization": organization.name, "account_type": "Receivable", "is_group": 0}),
		"default_payable_account": frappe.db.get_value("Account",
			{"organization": organization.name, "account_type": "Payable", "is_group": 0})
	})

	organization.save()
	install_country_fixtures(organization.name, organization.country)
	organization.create_default_tax_template()
