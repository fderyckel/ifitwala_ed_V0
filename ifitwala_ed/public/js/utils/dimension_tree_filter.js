...
ifitwala_ed.accounts.dimensions = {
		...
		if (frappe.meta.has_field(doctype, dimension)) {
			frappe.model.with_doctype(doctype, () => {
				...
				frm.set_query(dimension, parentfield, function(doc, cdt, cdn) {
					...
					return ifitwala_ed.queries.get_filtered_dimensions(row, fields, dimension, doc.organization);
				});
			});
		}
	},
	...
	setup_account_filters(frm, dimension, fields) {
		frm.set_query(dimension, function(doc) {
			return ifitwala_ed.queries.get_filtered_dimensions(doc, fields, dimension, doc.organization);
		});
	},
	...
	update_dimension(frm, doctype) {
		if (this.accounting_dimensions) {
			this.accounting_dimensions.forEach((dimension) => {
				if (frm.is_new()) {
					if (frm.doc.organization && Object.keys(this.default_dimensions || {}).length > 0
						&& this.default_dimensions[frm.doc.organization]) {

						let default_dimension = this.default_dimensions[frm.doc.organization][dimension['fieldname']];
