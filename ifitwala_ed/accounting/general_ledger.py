...
def validate_accounting_period(gl_map):
	accounting_periods = frappe.db.sql(""" SELECT
		...
		WHERE
			ap.name = cd.parent
			AND ap.organization = %(organization)s
			AND cd.closed = 1
			AND cd.document_type = %(voucher_type)s
			AND %(date)s between ap.start_date and ap.end_date
			""", {
				'date': gl_map[0].posting_date,
				'organization': gl_map[0].organization,
...
def merge_similar_entries(gl_map, precision=None):
	...
	organization = gl_map[0].organization if gl_map else ifitwala_ed.get_default_organization()
	organization_currency = ifitwala_ed.get_organization_currency(organization)

	if not precision:
		precision = get_field_precision(frappe.get_meta("GL Entry").get_field("debit"), organization_currency)
...
def round_off_debit_credit(gl_map):
	precision = get_field_precision(frappe.get_meta("GL Entry").get_field("debit"),
		currency=frappe.get_cached_value('Organization',  gl_map[0].organization,  "default_currency"))
...
def make_round_off_gle(gl_map, debit_credit_diff, precision):
	round_off_account, round_off_cost_center = get_round_off_account_and_cost_center(gl_map[0].organization)
...
def get_round_off_account_and_cost_center(organization):
	round_off_account, round_off_cost_center = frappe.get_cached_value('Organization',  organization,
