...
class Budget(Document):
	...
	def validate_duplicate(self):
		...
		existing_budget = frappe.db.sql("""
			...
			WHERE
				ba.parent = b.name and b.docstatus < 2 and b.organization = %s and %s=%s and
				b.fiscal_year=%s and b.name != %s and ba.account in (%s) """
				% ('%s', budget_against_field, '%s', '%s', '%s', ','.join(['%s'] * len(accounts))),
			(self.organization, budget_against, self.fiscal_year, self.name) + tuple(accounts), as_dict=1)
		...
		for d in self.get('accounts'):
			if d.account:
				...
				if account_details.is_group:
					...
					frappe.throw(_("Account {0} does not belongs to organization {1}")
						.format(d.account, self.organization))
...
def validate_expense_against_budget(args):
	...
	if args.get('organization') and not args.fiscal_year:
		args.fiscal_year = get_fiscal_year(args.get('posting_date'), organization=args.get('organization'))[0]
		frappe.flags.exception_approver_role = frappe.get_cached_value('Organization', args.get('organization'),  'exception_budget_approver_role')
...
def get_item_details(args):
	...
	if not args.get('organization'):
		return cost_center, expense_account

	if args.item_code:
		item_defaults = frappe.db.get_value('Item Default',
			{'parent': args.item_code, 'organization': args.get('organization')},
			['buying_cost_center', 'expense_account'])
		if item_defaults:
			cost_center, expense_account = item_defaults

	if not (cost_center and expense_account):
		for doctype in ['Item Group', 'Organization']:
...
def get_expense_cost_center(doctype, args):
	if doctype == 'Item Group':
		return frappe.db.get_value('Item Default',
			{'parent': args.get(frappe.scrub(doctype)), 'organization': args.get('organization')},
			['buying_cost_center', 'expense_account'])
	else:
		return frappe.db.get_value(doctype, args.get(frappe.scrub(doctype)), ['cost_center', 'default_expense_account'])
	...
	for budget in budget_records:
		if flt(budget.budget_amount):
			...
			if monthly_action in ["Stop", "Warn"]:
				...
				compare_expense_with_budget(args, budget_amount, _("Accumulated Monthly"), monthly_action, budget.budget_against, amount)

			if yearly_action in ("Stop", "Warn") and monthly_action != "Stop" and yearly_action != monthly_action:
				compare_expense_with_budget(args, flt(budget.budget_amount), _("Annual"), yearly_action, budget.budget_against, amount)

def get_amount(args, budget):
	amount = 0

	if args.get('doctype') == 'Material Request' and budget.for_material_request:
		amount = (get_requested_amount(args, budget) + get_ordered_amount(args, budget) + get_actual_expense(args))
...
def compare_expense_with_budget(args, budget_amount, action_for, action, budget_against, amount=0):
	...
	if actual_expense > budget_amount:
		...
		currency = frappe.get_cached_value('Organization',  args.organization,  'default_currency')
...
def get_actual_expense(args):
	...
	amount  = flt(frappe.db.sql("""
		...
		where gle.account=%(account)s
			...
			and gle.organization=%(organization)s
			and gle.docstatus=1
			{condition2}
