...
def make_tax_account_and_template(organization, account_name, tax_rate, template_name=None):
	...
	for i, name in enumerate(account_name):
		tax_account = make_tax_account(organization, account_name[i], tax_rate[i])
		if tax_account:
...
def make_tax_account(organization, account_name, tax_rate):
	tax_group = get_tax_account_group(organization)
	if tax_group:
		try:
			...
			if frappe.message_log: frappe.message_log.pop()
			abbr = frappe.get_cached_value('Organization',  organization,  'abbr')
...
def get_tax_account_group(organization):
	tax_group = frappe.db.get_value("Account",
		{"account_name": "Duties and Taxes", "is_group": 1, "organization": organization})
	if not tax_group:
