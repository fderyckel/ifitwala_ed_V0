...
def setup_taxes_and_charges(organization_name: str, country: str):
	...
	from_detailed_data(organization_name, country_wise_tax.get('chart_of_accounts'))
...
def from_detailed_data(organization_name, data):
	"""Create Taxes and Charges Templates from detailed data."""
	coa_name = frappe.db.get_value('Organization', organization_name, 'chart_of_accounts')
	...
	if sales_tax_templates:
		for template in sales_tax_templates:
			make_taxes_and_charges_template(organization_name, 'Sales Taxes and Charges Template', template)

	if purchase_tax_templates:
		for template in purchase_tax_templates:
			make_taxes_and_charges_template(organization_name, 'Purchase Taxes and Charges Template', template)

	if item_tax_templates:
		for template in item_tax_templates:
			make_item_tax_template(organization_name, template)


def make_taxes_and_charges_template(organization_name, doctype, template):
	template['organization'] = organization_name
	template['doctype'] = doctype

	if frappe.db.exists(doctype, {'title': template.get('title'), 'organization': organization_name}):
		return
	...
	for tax_row in template.get('taxes'):
		...
		if isinstance(account_data, dict):
			...
			account = get_or_create_account(organization_name, account_data)
...
def make_item_tax_template(organization_name, template):
...
def get_or_create_account(organization_name, account):
	"""
	...
	tax_group = get_or_create_tax_group(organization_name, root_type)
...
def get_or_create_tax_group(organization_name, root_type):
