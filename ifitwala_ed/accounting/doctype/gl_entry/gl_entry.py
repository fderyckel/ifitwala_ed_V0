...
import ifitwala_ed
from ifitwala_ed.accounting.doctype.accounting_dimension.accounting_dimension import (
	get_checks_for_pl_and_bs_accounts,
)
from ifitwala_ed.accounting.doctype.accounting_dimension_filter.accounting_dimension_filter import (
	get_dimension_filter_map,
)
from ifitwala_ed.accounting.party import validate_party_frozen_disabled, validate_party_gle_currency
from ifitwala_ed.accounting.utils import get_account_currency, get_fiscal_year
from ifitwala_ed.exceptions import (
...
class GLEntry(Document):
	...
	def check_mandatory(self):
		mandatory = ['account','voucher_type','voucher_no','organization']
		for k in mandatory:
			if not self.get(k):
	...
	def pl_must_have_cost_center(self):
		...
		if frappe.get_cached_value("Account", self.account, "report_type") == "Profit and Loss":
			...
			msg += " "
			msg += _("Please set the cost center field in {0} or setup a default Cost Center for the Organization.").format(
				self.voucher_type)
	...
	def validate_dimensions_for_pl_and_bs(self):
		...
		for dimension in get_checks_for_pl_and_bs_accounts():
			if account_type == "Profit and Loss" \
				and self.organization == dimension.organization and dimension.mandatory_for_pl and not dimension.disabled:
			...
			if account_type == "Balance Sheet" \
				and self.organization == dimension.organization and dimension.mandatory_for_bs and not dimension.disabled:
				if not self.get(dimension.fieldname):
	...
	def validate_account_details(self, adv_adj):
		...
		if ret.organization != self.organization:
			frappe.throw(_("{0} {1}: Account {2} does not belong to Organization {3}")
				.format(self.voucher_type, self.voucher_no, self.account, self.organization))

	def validate_cost_center(self):
		if not self.cost_center: return

		is_group, organization = frappe.get_cached_value('Cost Center',
			self.cost_center, ['is_group', 'organization'])

		if organization != self.organization:
			frappe.throw(_("{0} {1}: Cost Center {2} does not belong to Organization {3}")
				.format(self.voucher_type, self.voucher_no, self.cost_center, self.organization))
	...
	def validate_currency(self):
		...
		if not self.account_currency:
			self.account_currency = account_currency or organization_currency

		if account_currency != self.account_currency:
			frappe.throw(_("{0} {1}: Accounting Entry for {2} can only be made in currency: {3}")
				.format(self.voucher_type, self.voucher_no, self.account,
				(account_currency or organization_currency)), InvalidAccountCurrency)
	...
	def validate_and_set_fiscal_year(self):
		if not self.fiscal_year:
			self.fiscal_year = get_fiscal_year(self.posting_date, organization=self.organization)[0]
...
def update_against_account(voucher_type, voucher_no):
	entries = frappe.db.get_all("GL Entry",
		...
		fields=["name", "party", "against", "debit", "credit", "account", "organization"])
