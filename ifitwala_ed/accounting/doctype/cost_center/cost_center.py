...
class CostCenter(NestedSet):
	...
	def before_rename(self, olddn, newdn, merge=False):
		...
		from ifitwala_ed.setup.doctype.organization.organization import get_name_with_abbr
		...
		if not merge:
			...
			if new_parts[0][0].isdigit():
				...
				if new_cost_center.cost_center_number != new_parts[0]:
					validate_field_number("Cost Center", self.name, new_parts[0], self.organization, "cost_center_number")
	...
	def check_if_child_exists(self):
		return frappe.db.sql("select name from `tabCost Center` where parent_cost_center = %s and docstatus != 2", self.name)
